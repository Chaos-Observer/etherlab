/*
  ...
  |          |           | - Append PWM functions.                             |
  |          |           | - Append i_APCI1710_Read16BitCounterValue           |
  |          |           |          i_APCI1710_Write16BitCounterValue          |
  |          |           |          i_APCI1710_Write32BitCounterValue          |
  +----------+-----------+-----------------------------------------------------+  
  | 28.06.04 | J. Krauth | - Append functions prototypes and change            |
  |          |           |   structure names to be used in combination         |
  |          |           |   with other drivers.                               |
  +----------+-----------+-----------------------------------------------------+ 
  | 05.08.04 | J. Krauth | - Append ETM functions.                             | 
  |          |           | - Remove double.                                    |   
  +----------+-----------+-----------------------------------------------------+    
*/
...
typedef struct
{
  ...
  BYTE    b_TimingUnit;
  BYTE    b_CycleMode;
  // BEGIN JK 05.08.04: Remove all double
  //double d_TimingInterval;
  DWORD d_TimingInterval;
  // END JK 05.08.04: Remove all double	 
  DWORD  dw_ConfigReg;
}str_APCI1710_ChronoModuleInformation;
// END JK 08.09.03

// BEGIN JK 03.08.04 : Append ETM funtionality
      /*************/
      /* ETM infos */
      /*************/
...
// END JK 03.08.04 : Append ETM funtionality
...
typedef struct
{
  ...
  // END JK 08.09.03   
  // BEGIN JK 03.08.04 : Append ETM funtionality
  str_ETMModuleInfo s_ETMModuleInfo;                              /* ETM information */ 
  // END JK 03.08.04 : Append ETM funtionality
...
// BEGIN JK 26.07.04 : Append ETM funtionality
/*
+----------------------------------------------------------------------------+
| Function Name     : INT i_APCI1710_InitETM                                 |
|                                       (BYTE     b_BoardHandle,             |
|                                        BYTE     b_ModuleNumber,            |
|                                        BYTE     b_ClockSelection,          |
|                                        BYTE     b_TimingUnit,              |
...
+----------------------------------------------------------------------------+
| Task              : Configure all ETM (edge time measurement) from the     |
|		      selected module (b_ModulNbr). The ul_Timing,           |
|		      and ul_TimingUnit determine the timing base for the    |
|		      time measurement. pul_RealTiming  return the real      |
|		      timing value.                                          |
+----------------------------------------------------------------------------+
| Input Parameters  : BYTE      b_BoardHandle    : Handle of board APCI-1710 |
|                     BYTE      b_ModuleNumber   : Module number to configure|
...
|                                                       1 : æs               |
|                                                       2 : ms               |
|                     ULONG     ul_Timing        : Base timing value.        |
+----------------------------------------------------------------------------+
| Output Parameters : PULONG   pul_RealTiming     : Real Base timing value.  |
+----------------------------------------------------------------------------+
| Return Value      : 0: No error                                            |
|                    -1: The handle parameter of the board is wrong          |
|                    -2: Module selection wrong                              |
|                    -3: The module is not a ETM module                      |
|                    -4: The selected input clock is wrong                   |
|                    -5: Timing Unit selection is wrong                      |
|                    -6: Base timing selection is wrong                      |
|                    -7: You can not used the 40MHz clock selection with     |
|                        this board                                          |
+----------------------------------------------------------------------------+
*/
                                
INT	i_APCI1710_InitETM	(BYTE     b_BoardHandle,
				 BYTE     b_ModuleNumber,
				 BYTE     b_ClockSelection,
				 BYTE     b_TimingUnit,
				 ULONG   ul_Timing,
...
/*
+----------------------------------------------------------------------------+
| Function Name     : INT i_APCI1710_EnableETM                               |
|				(BYTE		 b_BoardHandle,              |
|				 BYTE		 b_ModuleNumber,             |
|				 BYTE		 b_ETM,                      |
|				 BYTE		 b_EdgeLevel,                |
...
|				 BYTE		 b_InterruptEnable)          |
+----------------------------------------------------------------------------+
| Task              : Enable the selected ETM (b_ETM) of the selected        |
...
|                     BYTE      b_ModuleNumber   : Module number to configure|
|                                                  (0 to 3)                  |
|                     BYTE 	b_ETM		 : Selected ETM (0 or 1)     |
|                     BYTE 	b_EdgeLevel	 : Selection from edge to    |
|						   measure the time          |
|						   0 : Measure the low level |
|						       time                  |
|						   1 : Measure the high level|
|						       time                  |
|                     BYTE 	b_TriggerLevel	 : Selection from trigger    |
|						   level                     |
|						   0 : The ETM trigger which |
|						       a low level           |
|						   1 : The ETM trigger which |
|						       a high level          |
|                     BYTE 	b_CycleMode	 : Selection from mode       |
|						   0 : Single mode           |
|						   1 : Continuous mode.      |
|						       Each trigger stop the |
|						       measurement a start a |
|						       new measurement cycle |
|                     BYTE 	b_FirstTriggerMode : First trigger mode      |
|						   0 : The first edge time   |
|						       measurement start     |
|						       after the calling     |
|						       from                  |
|						       "i_APCI1710_EnableETM"|
|						   1 : The first edge time   |
|						       measurement start     |
|						       after the next        |
|						       trigger signal        |
|                     BYTE 	b_InterruptEnable: Enable or disable the     |
...
|                    -2: Module selection wrong                              |
|                    -3: The module is not a ETM module                      |
|                    -4: ETM selection is wrong                              |
|                    -5: ETM not initialised, see function                   |
|			 "i_APCI1710_InitETM"                                |
|                    -6: Edge level selection is wrong                       |
|                    -7: Trigger level selection is wrong                    |
|                    -8: Mode selection is wrong                             |
|                    -9: First trigger mode selection is wrong               |
|                   -10: Interrupt parameter is wrong                        |
|                   -11: Interrupt function not initialised.                 |
|			 See function "i_APCI1710_SetBoardIntRoutineX"       |
+----------------------------------------------------------------------------+
...
INT	i_APCI1710_EnableETM	(BYTE		 b_BoardHandle,
				 BYTE		 b_ModuleNumber,
				 BYTE		 b_ETM,
				 BYTE		 b_EdgeLevel,
				 BYTE		 b_TriggerLevel,
...
/*
+----------------------------------------------------------------------------+
| Function Name     : INT   i_APCI1710_DisableETM                            |
|					(BYTE  b_BoardHandle,                |
|					 BYTE  b_ModuleNumber,               |
|					 BYTE  b_ETM)                        |
+----------------------------------------------------------------------------+
| Task              : Disable the selected ETM (b_ETM) of the selected       |
|		      module (b_ModulNbr).                                   |
+----------------------------------------------------------------------------+
| Input Parameters  : BYTE      b_BoardHandle    : Handle of board APCI-1710 |
|                     BYTE      b_ModuleNumber   : Module number to configure|
...
INT	i_APCI1710_DisableETM	(BYTE  b_BoardHandle,
				 BYTE  b_ModuleNumber,
				 BYTE  b_ETM);

/*				 
+----------------------------------------------------------------------------+
| Function Name     : INT   i_APCI1710_GetETMInitialisation                  |
|					(BYTE	   b_BoardHandle,            |
|					 BYTE	   b_ModuleNumber,           |
|					 BYTE	   b_ETM,                    |
|					 PBYTE	  pb_TimingUnit,             |
...
|					 PBYTE	  pb_Enable)                 |
+----------------------------------------------------------------------------+
| Task              : Return the ETM (b_ETM) initialisation of the selected  |
|		      module (b_ModuleNumber).                               |
|		      First call the function "i_APCI1710_InitETM" before    |
...
|							   1: æs             |
|							   2: ms             |
|		      PULONG 	pul_Timing	 : Base timing value.        |
|						   See base timing value     |
|						   description table         |
|		      PBYTE 	pb_EdgeLevel	 : Selection from edge to    |
|						   measure the time          |
|						   0 : Measure the low level |
|						       time                  |
|						   1 : Measure the high level|
|						       time                  |
|		      PBYTE 	pb_TriggerLevel	 : Selection from trigger    |
|						   level                     |
|						   0 : The ETM trigger which |
|						       a low level           |
|						   1 : The ETM trigger which |
|						       a high level          |
|		      PBYTE 	pb_CycleMode	 : Selection from mode       |
|						   0 : Single mode           |
|						   1 : Continuous mode.      |
|						       Each trigger stop the |
|						       measurement a start   |
|						       a new measurement     |
|						       cycle                 |
|		      PBYTE 	pb_FirstTriggerMode : First trigger mode     |
|						   0 : The first edge time   |
|						       measurement start     |
|						       after the calling     |
|						       from                  |
...
|						       after the next        |
|						       trigger signal        |
|		      PBYTE 	pb_InterruptEnable: Enable or disable the    |
|						    ETM interrupt.           |
|						    APCI1710_ENABLE :        |
|						    Enable the ETM interrupt |
|						    An interrupt occurs      |
...
|						    Disable the ETM          |
|						    interrupt                |
|		      PBYTE 	pb_Enable	  : Indicate if the ETM is   |
|						    enabled or not           |
|						    0: ETM not enabled       |
|						    1: ETM enabled           |
+----------------------------------------------------------------------------+
...
INT	i_APCI1710_GetETMInitialisation	(BYTE	   b_BoardHandle,
					 BYTE	   b_ModuleNumber,
					 BYTE	   b_ETM,
					 PBYTE	  pb_TimingUnit,
					 PULONG  pul_Timing,
...
/*
+----------------------------------------------------------------------------+
| Function Name     : INT   i_APCI1710_GetETMProgressStatus                  |
|				(BYTE	  b_BoardHandle,                     |
|				 BYTE     b_ModuleNumber,                    |
|				 BYTE	  b_ETM,                    	     |
|				 PBYTE	 pb_ETMtatus)                        |
+----------------------------------------------------------------------------+
| Task              : Returns the ETM status (pb_ETMtatus) of the selected   |
|		      ETM module (b_ModuleNumber).                           |
+----------------------------------------------------------------------------+
| Input Parameters  : BYTE      b_BoardHandle    : Handle of board APCI-1710 |
|                     BYTE      b_ModuleNumber   : Module number to configure|
|                                                  (0 to 3)                  |
|                     BYTE	b_ETM		 : Selected ETM (0 or 1)     |
+----------------------------------------------------------------------------+
| Output Parameters : PBYTE    pb_ETMtatus   : Returns the ETM status.       |
|					       0: Measurement not started.   |
|						  No start trigger signal    |
|						  occurs.                    |
|					       1: Measurement started.       |
|						  A start trigger signal     |
|						  occurs.                    |
|					       2: Measurement stopped.       |
|						  A stop trigger signal      |
|						  occurs.                    |
|						  The measurement is ended.  |
|					       3: An overflow occurs.        |
...
INT	i_APCI1710_GetETMProgressStatus	(BYTE	  b_BoardHandle,
					 BYTE    b_ModuleNumber,
					 BYTE	  b_ETM,
					 PBYTE	 pb_ETMtatus);
...
/*
+----------------------------------------------------------------------------+
| Function Name     : INT i_APCI1710_ReadETMValue                            |
|				(BYTE	   b_BoardHandle,                    |
|				 BYTE	   b_ModuleNumber,                   |
|				 BYTE	   b_ETM,                            |
|				 UINT	  ui_TimeOut,                        |
...
|				 PULONG   pul_ETMValue)                      |
+----------------------------------------------------------------------------+
| Task              : Returns the ETM status (pb_ETMtatus) and the time value|
...
|		      You can test the ETM status with                       |
|		      "i_APCI1710_GetETMProgressStatus".                     |
|                                                                            |
|		      The returned value of pul_ETMValue is not the actual   |
|		      measured time value.                                   |
|		      You must use the "i_APCI1710_ConvertETMValue" function |
...
|                                                  (0 to 3)                  |
|                     BYTE    b_ETM		 : Selected ETM (0 or 1)     |
|                     UINT    ui_TimeOut	 : Timeout selection         |
|						   (0 to 65535)              |
...
|						      the ETM status and if  |
|						      a stop signal occurs   |
|						      the measured time      |
|						      value.                 |
|						   1 to 65535: Determines    |
|						      the timeout in ms.     |
|						      The function return    |
...
|                    -7: Interrupt routine installed.                        |
|			 You cannot directly read the ETM measured time.     |
+----------------------------------------------------------------------------+
*/

INT	i_APCI1710_ReadETMValue	(BYTE	   b_BoardHandle,
				 BYTE	   b_ModuleNumber,
				 BYTE	   b_ETM,
				 UINT	  ui_TimeOut,
				 PBYTE	  pb_ETMtatus,
...
/*
+----------------------------------------------------------------------------+
| Function Name     : INT i_APCI1710_ReadETMTotalTime                        |
|				(BYTE	   b_BoardHandle,                    |
|				 BYTE	   b_ModuleNumber,                   |
|				 BYTE	   b_ETM,                            |
|				 UINT	  ui_TimeOut,                        |
...
|				 PULONG pul_ETMValue)                        |
+----------------------------------------------------------------------------+
| Task              : Returns the ETM status (pb_ETMtatus) and the total time|
|		      value (pul_ETMValue) after a stop signal occurred on   |
|		      the selected ETM module (b_ModulNbr). This function is |
|		      only available if you have disabled the interrupt      |
|		      function.                                              |
|		      See function "i_APCI1710_EnableETM" and table 3-1.     |
|		      You can test the ETM status with                       |
|		      "i_APCI1710_GetETMProgressStatus".                     |
...
|						      stop signal occur.     |
+----------------------------------------------------------------------------+
| Output Parameters : PULONG 	pul_ETMValue	: ETM total time value.      |
+----------------------------------------------------------------------------+
| Return Value      : 0: No error                                            |
|                    -1: The handle parameter of the board is wrong          |
|                    -2: Module selection wrong                              |
...
INT	i_APCI1710_ReadETMTotalTime	(BYTE	b_BoardHandle,
					 BYTE	b_ModuleNumber,
					 BYTE	b_ETM,
					 UINT	ui_TimeOut,
					 PBYTE	pb_ETMtatus,
