...
int msr_port = 2345;
uint32_t active_apps = 0;

struct rtp_io {
    /* The path to the character device of the rt_appcore */
    const char *dev_path;
	...
	int rtp_fd;             // File descriptor to the real time process
        ...
        /* Properties of the application. Defined in fio_ioctl.h */
         ...
         * structure */
        void *rtP;              // Pointer to application parameters
    } app[MAX_APPS];
...
{
	struct application *app = priv_data;
        struct param_change delta;
        int rv;

        delta.rtP = app->rtP;
        delta.pos = (void *)start - app->rtP;
        delta.len = len;
        delta.count = 0;

	if ((rv = ioctl(app->rtp_fd, CHANGE_PARAM, &delta)))
            perror("params");
...
static int __attribute__((used)) new_params_set(void *priv_data, char *start, size_t len)
{
	...
	return ioctl(app->rtp_fd, NEW_PARAM, app->rtP);
}
...
static void update_rtB(int fd, void *priv_data)
{
	...
	ssize_t len;
        ...
        if ((rv = ioctl(app->rtp_fd, GET_WRITE_PTRS, &data_p))) {
        }

        if (data_p.wp == -ENOSPC) {
            app->slice_rp = ioctl(app->rtp_fd, RESET_BLOCKIO_RP);
            fprintf(stderr, "Kernel buffer overflowed... Resetting\n");
        } else {
            for (i = app->slice_rp; i < data_p.wp; i++) {
                /* app->blockio_start contains the base pointer to the buffer
                 ...
                 *      | *app->blockio_start
                 *
                 * Then (*app->blockio_start)[i] is the i'th slice, and
                 * &(*app->blockio_start)[i] is its address. Call msr_update
                 * with this address
                 */
                msr_update(i);
            }

            app->slice_rp = ioctl(app->rtp_fd, SET_BLOCKIO_RP, data_p.wp);
        }

        while (data_p.msg > 0) {
            len = read(app->rtp_fd, msg_buf, sizeof(msg_buf-1));
            if (len < 0) {
...
static int start_app(const char *app_name, unsigned int app_num)
{
    int rv = 0;
    ...
    /* Get a pointer to app, going to need it often */
    app = &rtp_io.app[app_num];
     ...
     * numbers. */
    snprintf(filename, PATH_MAX, "%s%u", rtp_io.dev_path, app_num + 1);
    syslog(LOG_DEBUG, "Open data channel (%s) to the Real-Time Process\n",
            filename);
    app->rtp_fd = open(filename, O_RDONLY | O_NONBLOCK);
    CHECK_ERR(app->rtp_fd < 0, errno, out_open_rtb,
    ...
    rv = ioctl(app->rtp_fd, GET_APP_PROPERTIES, &app->properties);
    CHECK_ERR(rv, errno, out_get_app_properties,
            "Error: Could not fetch application properties: %s", strerror(errno));

    /* Get memory for parameter structure */
    app->rtP = malloc(app->properties.rtP_size);
    CHECK_ERR(!app->rtP, errno, out_malloc_rtP,
            "Could not allocate memory: %s", strerror(errno));

    /* Initialise it with the current Real-Time Process parameters. Note
     * that the application may already have been running and that these parameters
     * are not necessarily the defaults */
    rv = ioctl(app->rtp_fd, GET_PARAM, app->rtP);
    CHECK_ERR(rv < 0, errno, out_ioctl_get_param,
            "Could not fetch application's current parameter set: %s",
                strerror(errno));
    ...
    app->blockio_start = 
        mmap(0, app->properties.rtB_size * app->properties.rtB_count, 
                PROT_READ, MAP_PRIVATE, app->rtp_fd, 0);
    CHECK_ERR(app->blockio_start == MAP_FAILED, errno, out_mmap,
            "Could not memory map application's IO space: %s", strerror(errno));
    app->slice_rp = ioctl(app->rtp_fd, RESET_BLOCKIO_RP);

    /*======================================================================
     * Find out the application's tick period, and initialise MSR and
	 * application
     *======================================================================*/
     ...
     * object above */
    path_buf = (char*) malloc(app->properties.variable_path_len + 1);
    CHECK_ERR (!path_buf, errno, out_malloc_si,
            "Could not allocate memory: %s", strerror(errno));

    for (idx = 0; idx < app->properties.signal_count; idx++) {
        struct signal_info si = {
            .index = idx,
            .path = path_buf,
            .path_buf_len = app->properties.variable_path_len + 1,
        };

        rv = ioctl(app->rtp_fd, GET_SIGNAL_INFO, &si);
    ...
    for (idx = 0; idx < app->properties.param_count; idx++) {
        struct signal_info si = {
            .index = idx,
            .path = path_buf,
            .path_buf_len = app->properties.variable_path_len + 1,
        ...
        rv = ioctl(app->rtp_fd, GET_PARAM_INFO, &si);
    ...
    task_stats = (struct task_stats *) (app->properties.rtB_size - 
         app->properties.num_tasks*sizeof(struct task_stats));
    msr_reg_time(&task_stats[0].time);
    for (st = 0; st < app->properties.num_tasks; st++) {
    ...
    syslog(LOG_DEBUG, "Preparing msr module socket port: %i\n", 
            msr_port + app_num);
    app->net_fd = prepare_tcp(msr_port + app_num, new_msr_client, msr_read, 
                msr_write, NULL);
    if (app->net_fd < 0) {
	...
	/* Now register the application with the main dispatcher. When the file
	 * handle becomes readable (using select) the callback is executed */
    init_fd(app->rtp_fd, update_rtB, NULL, app);

    /* Free unused memory */
    free(path_buf);

    syslog(LOG_DEBUG, "Finished registering application with buddy");
    return 0;
...
out_malloc_si:
    msr_cleanup();
out_msr_init:
    munmap(app->blockio_start, 
            app->properties.rtB_size * app->properties.rtB_count);
out_mmap:
out_ioctl_get_param:
    free(app->rtP);
out_malloc_rtP:
out_get_app_properties:
    close(app->rtp_fd);
out_open_rtb:
    return rv;
}

static void open_application(int fd, void *d)
{
    int rv, i, j, pid;
    uint32_t new_apps, deleted_apps, running_apps; 
    struct rt_app_name rt_app_name;

    /* Ask the rt_appcore for the bitmask of running applications. The bits in
     * running_apps represent whether the application is running */
    rv = ioctl(fd, RTAC_GET_ACTIVE_APPS, &running_apps);
    if (rv) {
        syslog(LOG_ERR, "ioctl() error: Could not get the bit mask of "
                "active applications: %s", strerror(errno));
        return;
    }

    /* Get a list of applications that have started since last time 
     * it was checked */
    new_apps = running_apps & ~active_apps;
    for( i = 0; i < 32; i++) {
        if (!(new_apps & (0x01<<i))) {
            continue;
        }
        ...
        rv = ioctl(fd, RTAC_APP_NAME, &rt_app_name);
        if (rv) {
            syslog(LOG_ERR, "ioctl() error getting application name: %s",
                    strerror(errno));
            continue;
        }
        syslog(LOG_DEBUG, "Starting new application number %i, name: %s", 
                i, rt_app_name.name);
        ...
        if (!(pid = fork())) { 
            ...
            if (start_app(rt_app_name.name, i))
                exit(-1);
            return;
        } else if (pid > 0) {
            // parent

            //reg_sigchild(pid, &rtp_io->app[i]);
        } else { 
            // fork() failed
    ...
    /* Get a list of applications that have started since last time 
     * it was checked */
    deleted_apps = ~running_apps & active_apps;
    for( i = 0; i < 32; i++) {
        if (!(deleted_apps & (0x01<<i))) {
            continue;
        }

        syslog(LOG_DEBUG, "Stopping deleted application number %i, name: %s", 
                i, rt_app_name.name);
    }
...
int rtp_module_prepare(void)
{
    ...
    syslog(LOG_DEBUG, "%s(%s): Initialising RT-AppCore comms channel",
            __FILE__, __func__);

    /* Open a file descriptor to the rt_appcore. It is allways the first
     * one */
    snprintf(filename, sizeof(filename), "%s0", rtp_io.dev_path);
    syslog(LOG_INFO, "    Opening character device %s", filename);

    if ((fd = open(filename, O_RDWR | O_NONBLOCK)) < 0) {
        syslog(LOG_EMERG, "Could not open control channel to rt_appcore: %s",
                strerror(errno));
        rv = -errno;
        goto out_open_manager;
    }

    init_fd(fd, open_application, NULL, &rtp_io);

    open_application(fd, &rtp_io);
