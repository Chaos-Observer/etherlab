...
case 'set'
    ...
    if ~strcmp(convert(model), model)
        set_param(gcbh,'model',convert(model));
        return
    end
...
case 'check'
    ...
    ud = get_param(gcbh, 'UserData');
    if ~isfield(ud, 'SlaveConfig')
        map = struct(...
                'EL1004', 'EL1004_0010000A',...
                'EL1014', 'EL1014_00000000',...
                'EL1004v2', 'EL1004',...
                'EL1014v2', 'EL1014');
        warning('el1xxx:Upgrade', ...
            'Automatically upgrading %s to new slave management', gcb);
...
case 'update'
    rv = slave_config;
    update_devices(varargin{1}, rv{1}, rv{2});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function rv = convert(varargin)
% return:
%       nargin > 0: new model if exists, otherwise old name
%       ~nargin: list of old models
...
switch nargin
case 2
    pos = find(structfun(@(x) x(1) == varargin{1} && x(2) == varargin{2}, models));
    if isempty(pos)
        rv = [];
        return
    end
...
otherwise
    fields = fieldnames(models);
    obsolete = cellfun(@length, fields) > 10;
    sorted = vertcat(sort(fields(~obsolete)), sort(fields(obsolete)));
    rv = {sorted, convert};
    return
end
...
rv.product = product(1);
rv.revision = product(2);
rv.sm = {...
...
if strcmp(get_param(gcbh,'output_type'), 'Vector Output')
