...
function rv = prepare_config(model, output_type, status, dtype, ...
	scale, offset, filter, tau, type_1,  type_2, type_3, type_4, ...
	dcmode, dccustomer)
...
% Fill in Scale
    ...
    % Fill in Offsets
    if (isempty(offset) || numel(offset) == 1 || ...
        numel(offset) == number_elements)
        if strcmp(output_type,'Separate Outputs')
            for k = 1 : number_elements
                if numel(offset) == 1
